...
public class NewOrderTransaction implements Runnable {
    ...
    public void run() {
        ...
        try (final Session session = graphdb.session()) {
            try (final Transaction tx = session.beginTransaction()) {
                ...
                final double orderAmount = itemNumbers.stream()
                        .map(pairIdCount -> {
                            ...
                            if (!res.hasNext()) {
                                System.out.format("Item id '%d' does not exist in the database.", itemId);
                                tx.rollback();
                            }
                            final Record next = res.single();
                            return Pair.with(
                ...
                System.out.println("Placing order; amount is " + orderAmount);
